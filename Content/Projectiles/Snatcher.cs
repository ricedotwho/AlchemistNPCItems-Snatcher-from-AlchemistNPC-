...
namespace AlchemistNPCItems.Content.Projectiles
{
	public class Snatcher : ModProjectile
	{
		...
		public override void AI()
		{
			...
			if (player.dead || !player.HasBuff(ModContent.BuffType<Content.Buff.Snatcher>()))
			{
            ...
            if (conf.enableSSB == true)
            {
                if (modPlayer.SnatcherCounter >= 15000)
                {
                    ...
                    for (int i = 0; i < 200; i++)
                    {
                        ...
                        if (distance < 500f && target.catchItem == 0 && !target.dontTakeDamage && !target.friendly && target.active && player.HeldItem.damage > 0)
                        {
