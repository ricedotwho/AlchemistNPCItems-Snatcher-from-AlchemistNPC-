...
namespace AlchemistNPCItems.Content.Buff
{
	public class ProjCounter : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex)
		{
			...
			{
				Projectile.NewProjectile(player.GetSource_FromAI(), player.position.X-15, player.position.Y-100, 0f, 0f, Mod.Find<ModProjectile>("Globe199").Type, 0, 0, player.whoAmI);
			}
			counter++;
            for (int i = 0; i < 1000; i++)
            {
                ...
                if (distance < 130f && !target.friendly && target.active && target.hostile)
                {
                    ...
                    {
						player.AddBuff(Mod.Find<ModBuff>("GuarantCrit").Type, 2); //GuarantCrit
						((AlchemistNPCPlayer)player.GetModPlayer<AlchemistNPCPlayer>()).GC = true;
						SoundEngine.PlaySound(SoundID.Item93, player.position);
						Projectile.NewProjectile(player.GetSource_FromAI(), player.Center.X, player.Center.Y, 0f, 0f, Mod.Find<ModProjectile>("Counter").Type, 0, 6, Main.myPlayer, 0f, 0f);
                        counter = 0;
                    }
                }
			}
